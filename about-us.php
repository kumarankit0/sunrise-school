...
<div class="flex flex-col w-full bg-surface text-on-surface">
  ...
  <section class="relative w-full min-h-[80vh] lg:min-h-[85vh] py-20 lg:py-28 bg-primary text-on-primary flex items-center justify-center overflow-hidden">
    <div class="absolute inset-0 bg-cover bg-center opacity-65" style="background-image: url('<?= get_image('about', 'hero_banner', school_img('school_home2.webp')) ?>')"></div>
  ...
  <section class="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 -mt-10 sm:-mt-16 w-full">
    <div class="bg-surface-pure rounded-2xl shadow-[0_12px_28px_rgba(11,38,71,0.08)] p-4 sm:p-8 lg:p-12 grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8">
      <div class="flex flex-col items-center text-center">
        <span class="font-display-hero text-2xl sm:text-3xl lg:text-4xl font-bold text-secondary"><?= get_text('about', 'stat1_num', '20+') ?></span>
        <span class="font-eyebrow text-[10px] sm:text-xs md:text-sm text-on-surface-variant uppercase mt-1 sm:mt-2 font-bold leading-tight"><?= get_text('about', 'stat1_lbl', 'Years of Excellence') ?></span>
      </div>
      <div class="flex flex-col items-center text-center">
        <span class="font-display-hero text-2xl sm:text-3xl lg:text-4xl font-bold text-secondary"><?= get_text('about', 'stat2_num', '100%') ?></span>
        <span class="font-eyebrow text-[10px] sm:text-xs md:text-sm text-on-surface-variant uppercase mt-1 sm:mt-2 font-bold leading-tight"><?= get_text('about', 'stat2_lbl', 'Board Results') ?></span>
      </div>
      <div class="flex flex-col items-center text-center">
        <span class="font-display-hero text-2xl sm:text-3xl lg:text-4xl font-bold text-secondary"><?= get_text('about', 'stat3_num', '1,500+') ?></span>
        <span class="font-eyebrow text-[10px] sm:text-xs md:text-sm text-on-surface-variant uppercase mt-1 sm:mt-2 font-bold leading-tight"><?= get_text('about', 'stat3_lbl', 'Alumni &amp; Students') ?></span>
      </div>
      <div class="flex flex-col items-center text-center">
        <span class="font-display-hero text-2xl sm:text-3xl lg:text-4xl font-bold text-secondary"><?= get_text('about', 'stat4_num', '1:15') ?></span>
        <span class="font-eyebrow text-[10px] sm:text-xs md:text-sm text-on-surface-variant uppercase mt-1 sm:mt-2 font-bold leading-tight"><?= get_text('about', 'stat4_lbl', 'Teacher-Student Ratio') ?></span>
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 lg:px-12 py-24 w-full">
    <div class="text-center max-w-3xl mx-auto mb-16">
      ...
      <h2 class="font-headline-lg text-headline-lg text-primary"><?= get_text('about', 'philosophy_heading', 'Guiding Principles of Education') ?></h2>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
      <div class="bg-surface-pure rounded-xl p-10 shadow-[0_2px_8px_rgba(11,38,71,0.04)] hover:shadow-[0_12px_28px_rgba(11,38,71,0.08)] transition-all flex flex-col justify-between border border-border-warm">
        <div class="flex flex-col gap-6">
          ...
          <h3 class="font-headline-md text-headline-md text-primary"><?= get_text('about', 'vision_title', 'To Enlighten Every Mind Like the Rising Sun') ?></h3>
          <p class="font-body-lg text-on-surface-variant">
            <?= get_text('about', 'vision_text', 'Sun Rise Sr. Sec. School envisions building a forward-looking student community rooted in timeless values, academic distinction, ethical integrity, and technological readiness to lead with compassion and confidence.') ?>
          </p>
        </div>
  ...
  <section class="bg-surface-container-low py-24 w-full">
    <div class="max-w-7xl mx-auto px-6 lg:px-12">
      <div class="text-center max-w-3xl mx-auto mb-20">
        <span class="text-eyebrow text-secondary uppercase font-eyebrow mb-2 block font-bold"><?= get_text('about', 'history_eyebrow', 'Our Growth Journey') ?></span>
        <h2 class="font-headline-lg text-headline-lg text-primary"><?= get_text('about', 'history_heading', 'Milestones in Our History') ?></h2>
      </div>
      <div class="relative border-l-2 border-border-warm ml-4 md:ml-32 pl-8 md:pl-12 flex flex-col gap-16">
        <div class="relative flex flex-col gap-2">
          ...
          <span class="text-eyebrow text-secondary font-eyebrow font-bold"><?= get_text('about', 'm1_tag', 'Foundation') ?></span>
          <h3 class="font-headline-md text-headline-md text-primary"><?= get_text('about', 'm1_title', 'Establishment of Sun Rise School') ?></h3>
          <p class="font-body-md text-on-surface-variant max-w-2xl">
            <?= get_text('about', 'm1_desc', 'Founded with the noble aspiration to bring quality, modern, value-based English and Hindi medium education to the youth of Dobhi and surrounding regions.') ?>
          </p>
        </div>
        <div class="relative flex flex-col gap-2">
          <div class="absolute -left-[41px] md:-left-[57px] top-1 w-6 h-6 rounded-full bg-primary border-4 border-surface-container-low flex items-center justify-center"></div>
        ...
        <div class="relative flex flex-col gap-2">
          <div class="absolute -left-[41px] md:-left-[57px] top-1 w-6 h-6 rounded-full bg-primary border-4 border-surface-container-low flex items-center justify-center"></div>
          <span class="text-eyebrow text-secondary font-eyebrow font-bold"><?= get_text('about', 'm3_tag', 'Infrastructure') ?></span>
          <h3 class="font-headline-md text-headline-md text-primary"><?= get_text('about', 'm3_title', 'Modern Science Labs, Computer Lab &amp; Sports Ground') ?></h3>
          <p class="font-body-md text-on-surface-variant max-w-2xl">
            <?= get_text('about', 'm3_desc', 'Inauguration of modern digital smart classrooms, a high-tech computer laboratory, a wide athletic sports ground, and annual science exhibitions.') ?>
          </p>
        </div>
        <div class="relative flex flex-col gap-2">
          <div class="absolute -left-[41px] md:-left-[57px] top-1 w-6 h-6 rounded-full bg-primary border-4 border-surface-container-low flex items-center justify-center"></div>
          <span class="text-eyebrow text-secondary font-eyebrow font-bold"><?= get_text('about', 'm4_tag', 'Today') ?></span>
          <h3 class="font-headline-md text-headline-md text-primary"><?= get_text('about', 'm4_title', 'Empowering Future Generations') ?></h3>
          <p class="font-body-md text-on-surface-variant max-w-2xl">
            <?= get_text('about', 'm4_desc', 'Consistently achieving 100% board results, district academic merit awards, and sports championships, paving pathways to premier universities and professional colleges.') ?>
          </p>
        </div>
      </div>
    </div>
  ...
  <section class="max-w-7xl mx-auto px-6 lg:px-12 py-24 w-full">
    <div class="bg-primary text-on-primary rounded-2xl p-8 lg:p-16 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
      ...
      <div class="lg:col-span-7 flex flex-col gap-6">
        <span class="text-eyebrow text-secondary-container uppercase tracking-widest font-eyebrow font-bold"><?= get_text('about', 'leader_tagline', 'Leadership Message') ?></span>
        <blockquote class="font-headline-lg text-headline-lg text-on-primary italic font-serif">
          <?= get_text('about', 'leader_quote', '“Our mission is to illuminate young minds with knowledge, strengthen their character with discipline, and empower them to become successful citizens of tomorrow.”') ?>
        </blockquote>
        <div class="flex flex-col gap-1 mt-4">
          <span class="font-label-md text-on-primary font-bold"><?= get_text('about', 'leader_title', 'Principal &amp; Management Committee') ?></span>
          <span class="text-body-sm text-primary-fixed-dim"><?= get_text('about', 'leader_sub', 'Sun Rise Sr. Sec. School, Dobhi (Hisar, Haryana)') ?></span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 lg:px-12 pb-24 w-full">
    <div class="text-center max-w-3xl mx-auto mb-16">
      ...
      <h2 class="font-headline-lg text-headline-lg text-primary"><?= get_text('about', 'tour_title', 'Moments &amp; Campus Life') ?></h2>
    </div>
